...
import {getAvatar, getName, getLastSeen} from "../../../../utils";
...
import ConversationInfoModal
	from "../../modals/ConversationInfoModal/ConversationInfoModal.vue";
...
<template>
	<div>
		...
		<div class="w-full px-5 py-6 flex justify-center items-center">
			...
			<div v-if="chat.status !== 'loading'" class="flex grow">
				...
				<div class="flex flex-col">
					...
					<Typography
						variant="body-2" class="font-extralight" tabindex="0"
			...
			<div class="flex" :class="{'hidden': chat.status === 'loading'}">
				...
				<div class="relative">
					...
					<IconButton
						v-if="props?.activeConversation?.type !== 'group'"
						id="open-conversation-menu" @click="showDropdown = !showDropdown" tabindex="0"
					...
					>
						...
						<DropdownLink
							:handle-click="handleCloseDropdown"
		...
		<!--Search modal-->
		<SearchModal
			:open="openSearch" :close-modal="() => openSearch = false"
			:conversation="(props.activeConversation as Conversation)"
		/>

		<!--Contact info modal-->
		<ConversationInfoModal
			:open="openInfo" :closeModal="() => openInfo = false"
			:conversation="(props.activeConversation as Conversation)"
		/>
	</div>
</template>
