...
import { updateAccount } from "../../../../stores/actions";
...
const accordionState: Ref<boolean[]> = ref((settings.settings as Settings[]).map(() => true));

// array contains the values that determine wheter the accordion is loading or not.
const accordionLoading: Ref<boolean[]> = ref((settings.settings as Settings[]).map(() => false));
...
const updateSettings = (event: Value) => {
    ...
    settings.settings = newSettings.map((settingGroup) => {
...
<template>
    <div role="list" aria-label="Settings Accordion Control Group Buttons" class="h-full ">
        <AccordionElement v-for="(settingsGroup, index) in settings.settings" :settings-group="settingsGroup"
